...
5. `change` – prefer the semantic current-footprint union for baseline presence;
   ...
   source features as one event); suppress incomplete border-clipped change
   claims but retain unchanged presence; otherwise fall back to SAM discovery;
   produce `change.geojson` in pixel space or georeferenced coordinates
